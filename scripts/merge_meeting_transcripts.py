...
class MeetingTranscriptMerger:
    def __init__(self, input_file="icsi_meeting_dataset.csv"):
    ...
    def save_individual_transcripts(self, output_dir="meeting_transcripts"):
        ...
        for meeting_id in sorted(self.df['meeting_id'].unique()):
            ...
            lines = [
                f"=" * 80,
                f"ICSI Meeting Transcript: {meeting_id}",
                f"=" * 80,
                f"",
...
def main():
    print("=" * 80)
    print(" " * 20 + "ICSI MEETING TRANSCRIPT MERGER")
    print("=" * 80)
    
    # Merger oluştur
    merger = MeetingTranscriptMerger("data/raw/icsi_meeting_dataset.csv")
    ...
    # Excel'e kaydet
    merger.save_to_excel(merged_df, detailed_df, flow_df, "data/raw/icsi_merged_meetings.xlsx")
    
    # Bireysel transkript dosyaları oluştur
    merger.save_individual_transcripts("data/transcripts/icsi")
    ...
    print(f"  • icsi_merged_meetings.xlsx (4 sheet: Summary, Detailed, Flow, Text Only)")
    print(f"  • meeting_transcripts/ klasöründe {len(merged_df)} adet .txt dosyası")
    print("=" * 80)
